...
- **141 document tools** — read content, edit text, manage styles, insert images, handle tables, charts, conditional formatting, hyperlinks, track changes, page headers/footers, navigate headings, search, and more. The list is filtered by the active document type, so a client sees only what applies (95 with a Writer document, 48 with Calc)
...
|---|---|---|
| **Document type** | The tool list follows the active document — about 95 tools for Writer, 48 for Calc, out of 141. Automatic, and clients are notified when it changes | nothing to configure |
