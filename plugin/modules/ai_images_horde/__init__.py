...
"""AI Horde image generation sub-module."""
...
log = logging.getLogger("nelson.ai_images.horde")


class AiImagesHordeModule(ModuleBase):
    ...
    def initialize(self, services):
        ...
        for item in items:
            ...
            instance_id = "horde:%s" % name
            svc.register_instance(instance_id, ImageInstance(
                name=name,
                module_name="ai_images.horde",
                provider=_LazyProvider(item),
            ))


class _LazyProvider:
    """Deferred AI Horde provider — avoids import at startup."""
    ...
    def _ensure(self):
        if self._real is None:
            from plugin.modules.ai_images_horde.provider import (
                HordeImageProvider)
            self._real = HordeImageProvider(self._config)
        return self._real
