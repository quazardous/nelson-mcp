...
class WriteCellRange(ToolBase):
    ...
    description = (
        ...
        "A single formula on a range is filled like dragging the fill "
        "handle: written in the top-left cell, relative references shift "
        "row by row and column by column (C2, C3, …); use $ to keep one "
        "fixed ($C$2). A single constant is copied as is. "
